...
- **T1. CORRECTED BY LATER MEASUREMENT, read the correction first.**

  > **Correction (expD11 lead_B).** T1 as originally written is wrong. Scored against the *drifted problem's own* floor, stale block-QR whitening sits at 0.5-1.5x that floor across drift $\eta = 0 \ldots 10^{-8}$, while block-Jacobi steering is 180x to $2\times10^{6}$x above it. The original "$10^{-8}$ drift = 8 orders" was measuring the problem's approximation floor moving, not the method degrading. The baked-in factor is the better of the two at every $\eta$ tested, which weakens the case for a separate steering tier. Numbers: `results/checkpoint_D_optimizers/expD11_batching/lead_B_results.md`.

- **T1 (original, superseded). Steering vs. baked-in preconditioners.** A preconditioner that only steers (block-Jacobi, sketch+IR) degrades gracefully when stale or batch-estimated: cost appears as iterations. A factorization baked into the solution map (whitening) is poisoned by staleness ($10^{-8}$ drift = 8 orders, measured). Both step-2 types are steering-type *on purpose*; do not "optimize" either into a baked-in form.
