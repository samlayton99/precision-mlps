...
A [finite-slope proof for the uniform whole-line dictionary](../../../docs/neighbor_difference_conditioning.md) now gives explicit bounds: after dividing singular values by $\sqrt h$, the unscaled difference spectrum is bounded above and below independently of width at fixed $\lambda$. On the zero-total-readout subspace, the physical tanh representation instead has a condition number growing linearly with width. The proof therefore identifies a cumulative-coordinate penalty removed by differencing. It also shows that small-$\lambda$ smoothing remains: at $\lambda=0.25$, the ideal infinite-lattice singular-value condition number lies between approximately $1.05\times10^7$ and $1.49\times10^7$. These are analytic lattice bounds, not measured condition numbers of the training matrix.

The implemented system additionally needs bounds for the finite observation window, halo, bias/anchor, discrete sampling, and cumulative scales $S$. We have not established a full-system bound or proved that its scaling is optimal. The observed 32–37-fold increase in the median relative singular value describes the middle of the experimental spectrum; it does not establish a well-conditioned full matrix, whose smallest computed singular values remain near or below floating-point resolution.
...
Retained evidence includes complete per-step scalar traces; 20k checkpoints with parameters and Adam moments; selected dense analysis exports of physical parameters, signed gradients, and actual updates; Fourier-band residuals and forces; fixed-basis singular-mode projections; core/halo contributions; coefficient norms; and exact readout/geometry/interaction contributions to finite-step MSE changes. The full remote dense trajectory windows were retired as recorded below. Detached fits use three cutoffs and a doubled sampling density. Per-seed movies place physical $w$ above physical $\gamma$ at fixed centers and slow the first 320k checkpoints.
...
Slurm accounting records **28,069 GPU-seconds, or 7.79694 GPU-hours**, including compilation, I/O, the controlled cancellation used to replace slow compressed dense saves, and the final numerical audit. Allocation start/end times verify a maximum of two concurrent GPUs. All campaign jobs have ended. Remote analyses used zero-GPU Slurm allocations; local checks and evidence curation used CPUs. The 731 remaining GPU-seconds are unused budget, not an unreported allocation. Existing historical training is excluded from the new charge. Checkpoints, scalar traces, selected dense analysis exports, figures, and movies are retained locally.

On 2026-09-21 UTC, the user authorized cleanup of the Runpod ratios archive. Only the 5,539 files matching `dense_[nine digits]_[nine digits].npz` under `/workspace/junmiaoh/experiments/precision-mlps/runs/ratios/` were deleted: **877,546,003,295 bytes (817.3 GiB)**. The 24,975 other files, totaling 7.43 GiB, retained their paths, sizes, modification times, and inodes. Scalar traces, resumable checkpoints, metadata, separate analysis directories, and the gamma study archive were preserved. All 42 files directly linked from this report were present locally and hashed before deletion; this checks availability, not fresh regeneration of the analyses. The [completion record](ratio_dense_retirement/completion.json), [full before-deletion inventory](ratio_dense_retirement/inventory.json.gz), [deleted-path ledger](ratio_dense_retirement/deleted_paths.jsonl), and [retained report-input hashes](ratio_dense_retirement/retained_report_inputs.json) document the operation. Report-linked artifacts remain available, but new diagnostics requiring the retired full dense windows now require regenerating those windows.
