...
Does slowing geometry let the readout continue learning, or does it remove a useful route for reducing the residual? **None of the three late rate interventions improves the mean training MSE in any of the 12 width/target/seed cases at 1.1 million updates.** Raising the readout rate often increases occasional error excursions while barely changing the median error. The early handoffs and feedback policy also fail to improve their matched outcomes. Joint-training phases are complete; frozen-readout continuations and final analysis remain in progress. Two seeds establish paired observations, not a population-level significance claim.
...
Geometry's smaller motion makes more useful progress in the weak directions. Projecting the finite updates onto $s<10^{-4}$ at 1.1m, its sampled mean linear MSE reduction is 11–15 times the readout reduction on sine/1024 and 85–90 times on mixed/512. The projected geometry-only MSE change is negative in every one of the 64 sampled states in all 12 shared controls, with negligible quadratic cost in this band. Removing modes below relative singular value $10^{-10}$ changes the reported block means by less than 1%. These sampled budgets explain why small geometry motion can matter despite much larger readout travel; they are not complete-window drift estimates.
...
All 60 primary branches completed 1.1m updates; a subset reached 1.12m before the worker budgets expired. Comparisons use the common 1.1m horizon. All four early handoffs completed their planned 340k horizon, all 12 feedback cases completed the common 400k comparison, and both historical higher-rate tails reached 1.68m. Detailed 340k spectral and motion measurements are retained with their explicit horizon and checked against the final primary export. Longer frozen-dictionary solves and their final analysis remain pending in this interim report. No branch is declared converged merely because a budget limit or reporting horizon was reached.
