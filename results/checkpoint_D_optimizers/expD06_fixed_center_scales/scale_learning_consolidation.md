...
<figure>
  <video controls preload="metadata" poster="animations/seed_0_overview_last.png" style="max-width: 100%;" aria-label="Seed zero: physical readout weights on the top row and physical slopes on the bottom row, moving at their fixed centers through training">
    <source src="animations/seed_0_overview.mp4" type="video/mp4">
    Seed 0 animation: open the linked MP4 or interactive player below.
  </video>
  <figcaption>Figure 2a. Seed 0: physical readout weights w on the top row and physical slopes gamma on the bottom row, synchronized at all 559 fixed centers. Playback follows the shared-rate scaled Adam trajectory from initialization through 320k, then the joint-decay continuation to 1.68m. Blue circles mark core neurons; orange triangles mark halo neurons. Bias is displayed separately. Each parameter's vertical scale stays fixed and is logarithmic outside a small linear region around zero. Checkpoints through 320k are held for one second each; later checkpoints play at six per second. Checkpoint gaps are unequal, so the displayed update counter identifies training progress.</figcaption>
</figure>

<figure>
  <video controls preload="metadata" poster="animations/seed_1_overview_last.png" style="max-width: 100%;" aria-label="Seed one: physical readout weights on the top row and physical slopes on the bottom row, moving at their fixed centers through training">
    <source src="animations/seed_1_overview.mp4" type="video/mp4">
    Seed 1 animation: open the linked MP4 or interactive player below.
  </video>
  <figcaption>Figure 2b. Seed 1, with the same row order, vertical scales, checkpoints, and playback cadence as Figure 2a. Horizontal positions never move, and signs are retained as trained. Both movies show only recorded states without interpolation. Geometry continues to move after accurate detached fits are available, but its late movement is small on this full-trajectory scale.</figcaption>
</figure>

Open the interactive players for [seed 0](animations/seed_0.html) and [seed 1](animations/seed_1.html) to pause, step through frames, scrub the timeline, or change playback speed. Direct videos are also available: [seed 0 overview](animations/seed_0_overview.mp4) and [seed 1 overview](animations/seed_1_overview.mp4). The first 300k updates, including the next saved checkpoint at 320k, play six times more slowly. Each page includes a second, magnified movie of both parameters during total updates 1,677,952–1,679,999: [seed 0 late movement](animations/seed_0_late.mp4), [seed 1 late movement](animations/seed_1_late.mp4). These subtract the window's initial values, keep a fixed vertical scale, and show every eighth recorded state plus the final state. They illustrate motion; the complete per-update traces remain the evidence for oscillation statistics.
