# Exp0D -- Optimal $\lambda$ vs target frequency (QI vs lstsq)
...
Code: `experiments/exp0D_lambda_frequency/` (`run.py`, `config.yaml`, `sweep_utils.py`; tests in `tests/test_exp0D.py`). Run: `python3 experiments/exp0D_lambda_frequency/run.py`.
