# Exp04 -- QI vs Lstsq Coefficient Closeness

**Question:** on a *fixed* tanh geometry (shared centers and bandwidth), do QI and least-squares find the *same solution*?

Code: `experiments/exp04_coeff_nullspace/` (`run.py`, `config.yaml`, `coeff_compare.py`; tests in `tests/test_exp04.py`). Run: `python3 experiments/exp04_coeff_nullspace/run.py`.
