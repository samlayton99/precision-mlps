...
- **y-noise** -- a toggle + log-scale slider ($\sigma=10^k$) that adds Gaussian noise to the training targets (least-squares only, fixed seed). This exposes the conditioning: the masked-gap reconstruction is numerical analytic continuation, which amplifies error by up to $\mathrm{cond}(\Phi)\sim 10^{13}$. Noise-free the gap sits at $\sim 10^{-2}$; $\sigma=10^{-6}$ pushes it to $\sim 10^{4}$; the coefficient norm explodes from $\sim 0.5$ to $\sim 10^8$ -- the noise-free assumption is load-bearing (cf. expB01).
- **regularization (rcond)** -- log-scale slider for the SVD truncation floor ($\mathrm{rcond}=10^k$; default $10^{-13}$ = essentially none). The precision-vs-robustness dial: the near-null modes that reach machine precision on exact data are the same ones that amplify noise. Under $\sigma=10^{-4}$ noise, raising rcond from $10^{-13}$ to $10^{-4}$ pulls the masked gap from $\sim 10^6$ back to $\sim 10^{-1}$ -- but caps the clean-data precision at $\sim$ rcond. You cannot have both.
