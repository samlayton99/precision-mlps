# Exp05 -- GELU vs tanh feature-matrix conditioning
...
Code: `experiments/exp05_activation_conditioning/` (`run.py`, `config.yaml`, `gelu_phi.py`; tests in `tests/test_exp05.py`). Run: `python3 experiments/exp05_activation_conditioning/run.py`.

GELU has no QI construction (its derivative $\mathrm{GELU}'(z) = \Phi(z) + z\,\phi(z)$ is a sigmoid, not a localized bump), so the QI-dependent metrics of exp04 (`ratio_full`, `ratio_row`, `qi_fit`) do not apply. This experiment studies what *is* defined for GELU: $\Phi$ conditioning and the lstsq fit, with tanh overlaid for contrast.
...
**Step 1.** Neither activation has a *sharp* $\lambda$ optimum on this (low-frequency) target -- both lstsq curves are flat-bottomed until the high-$\lambda$ fp64-cancellation wall (consistent with exp06/exp02, where lstsq has a wide flat bottom and no fixed optimum). GELU's error is flat at $\sim 6\times10^{-13}$ to $5\times10^{-12}$ across the range; its minimum happens to fall at $\lambda = 0.220$ ($6.3\times10^{-13}$), but that is a representative point in the flat band, not a true optimum -- we adopt it as the fixed $\lambda$ for step 2. tanh is flat at $\sim 1\times10^{-13}$ over $\lambda \in [0.1, 0.3]$ then rises to $\sim 2\times10^{-9}$ by $\lambda = 0.5$.
...
- GELU yields a **more rank-deficient** $\Phi$ than tanh on the same geometry (effective rank $\approx 0.43N$ vs $\approx N$; null dimension growing with $N$ vs roughly constant). It does not reduce the coefficient underdetermination of exp04 -- it enlarges it.
