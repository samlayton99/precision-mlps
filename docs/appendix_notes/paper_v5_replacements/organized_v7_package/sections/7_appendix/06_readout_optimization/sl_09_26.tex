% Appendix E. Independent of the precision-experiment implementation.
\section{Frozen-geometry readout optimization}
\label{app:note-spectrum}

We prove Theorem~\ref{thm:note-output} by bounding the finite kernel's
eigenvalue ratios. A continuous-center matrix exposes their slope dependence;
explicit lattice and exterior-center corrections bound the finite network.

\subsection{Gradient descent and the exact residual}
...
\subsection{Continuous-center matrix and Fourier representation}
...
large for $|\omega|\gg\gamma$. Since $M_\gamma$ increases with $\gamma$,
...
\subsection{Finite-center corrections}
...
\subsection{Finite-kernel eigenvalue bounds}
...
\subsection{Output-error and training-time bounds}
The upper endpoints $\overline\rho_i$ above are denoted $\rho_i$
in the informal statement of Theorem~\ref{thm:note-output};
here $\rho_i=\mu_i/\mu_1$ denotes the exact ratio. Since $(1-\rho/2)^{2n}$ decreases on
...
\subsection{Numerical example and implementation}
