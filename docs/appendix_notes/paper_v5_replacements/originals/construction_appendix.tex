...
admissible. Suppose the numerical conditions in Section~\ref{sec:recovery}
and the sampling and input-rounding conditions in
...
Sections~\ref{sec:local}--\ref{sec:witness} construct the tanh reference and
bound its coefficients. Section~\ref{app:gelu-silu} gives the two activation
extensions. Section~\ref{sec:recovery} applies reference-scaled truncated SVD,
Section~\ref{sec:continuous} transfers the sampled bounds to continuous error,
and Section~\ref{sec:assemble} assembles the theorem and its width corollary.

\subsection{Local analytic tanh representation}
\label{sec:local}
Set
\begin{equation}
...
\subsection{Midpoint discretization}
...
\subsection{Boundary correction using the existing halo}
\label{sec:boundary}
Set
\begin{equation}
...
\subsection{Width-independent readout control}
...
\subsection{The analytic tanh witness}
\label{sec:witness}
...
Lemma~\ref{lem:midpoint}. The polynomial and its derivatives occur only in
the reference construction; the sampled algorithm below uses real values of
$f$ and deterministic coefficient envelopes.
...
\subsection{Reference-scaled truncated-SVD recovery}
\label{sec:recovery}
The same sampled recovery argument applies to all three dictionaries.
Assume $M\ge n$, where $n$ is the total readout dimension of the selected
dictionary, including its bias and any affine-correction units. Write the native features as $\phi_j$, with $\phi_b=1$, and set
\begin{equation}
...
\end{equation}
For tanh, $m_j=1$; for GELU/SiLU use
\qeqref{eq:psi-column-normalization}. In either case $|\varphi_j(x)|\le1$
on $\Om$ and $m_j\ge1$.

\subsubsection{Reference envelopes}
Let $E_W^{\rm an}$ denote the explicit analytic bound
\qeqref{eq:Erep}, \qeqref{eq:gelu-witness}, or \qeqref{eq:silu-witness}.
The witness has positive coordinate envelopes
\begin{equation}
 |v_j^\star|\le B\alpha_j,\qquad
 K_W^{\rm ref}:=\sum_j\alpha_j\le K_{\rm ref}.
 \label{eq:coordinate-envelope}
\end{equation}
For GELU/SiLU these are \qeqref{eq:psi-coordinate-envelopes}.
For tanh, put $m=\lfloor(R+1)/2\rfloor$ and
\begin{equation}
...
\end{proof}
At fixed observation density, the projected-vector allowance $e_p$ has a
pairwise contribution $O(u\sqrt W\log W)$ times $U_+Y$. This statement
concerns that contribution, not an assertion that every factorization,
storage, or input-rounding term has the same width dependence.

\subsection{From sampled error to continuous error}
...
\[
 ...
 \qquad(|v|\le\pi/4).
\]
For GELU/SiLU apply Lemma~\ref{lem:psi-strip-growth}. Thus there is a
width-independent $C_{\rm ell}$, equal to one for tanh and
$C_{\psi,\pi/4}(3+\lambda t_\varrho)$ for GELU/SiLU, with
\begin{equation}
...
\subsubsection{Input rounding and evaluation}
...
\subsection{Theorem assembly and width dependence}
\label{sec:assemble}
Put
\begin{equation}
...
For GELU/SiLU, use $E_W^{\rm an}=E_W^\psi$ and the envelopes
\qeqref{eq:psi-coordinate-envelopes}. The same computation proves
\begin{equation}
...
the activation-specific midpoint and numerical allowances. The analytical
high derivatives and complex target values are not evaluated by this sampled
construction. Its guarantees concern successful runs satisfying the stated
numerical bounds; they do not assert that every default least-squares
implementation has those bounds.
