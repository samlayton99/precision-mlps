...
% Append within Construction theory, after the width corollary.
...
\subsubsection{Classical staircase}
...
\subsubsection{A polynomial-based tanh specialization}
...
\subsubsection{Sampled sigmoidal quasi-interpolation}
...
\subsubsection{Measured comparison at fixed precision}
\label{app:measured-method-comparison}

Figure~\ref{fig:appendix-method-comparison} complements
Table~\ref{tab:comparison} with measured error versus total
hidden-neuron budget at two working precisions. At smaller total neuron budgets, QUILL achieves lower error than
ChebNet on these targets, using a single hidden layer of tanh units, prescribed feature geometry, and a linear readout solve. ChebNet attains lower final errors at larger budgets; QUILL's advantage here is its accuracy with fewer neurons and its simple, shallow construction.
