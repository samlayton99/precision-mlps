...
\author{}\date{Section 3 source note 4 \;---\; 25 September 2026}
\begin{document}
\maketitle
\begin{abstract}
...
\paragraph{How the argument fits together.}
There are four moves. First, compare a finite tanh sum with a center integral; contour integration separates the error caused by the center grid from the boundary and representation errors. Second, calculate the grid contribution from the poles of tanh: this is where $e^{-\pi^2/\lambda}$ comes from. Third, preserve the target-frequency dependence in the first pair of replicated frequencies, producing the $\sinh$ factor in the selection rule. Finally, choose the largest bandwidth allowed by an aliasing budget, and keep the remaining approximation and numerical budgets separate. The selection rule is inexpensive because it solves one scalar equation before fitting the readout.

\paragraph{What is proved and what is chosen.}
The finite-contour bounds, first-pair comparison, scalar inversion, and recovery transfer below are mathematical results. Replacing a target's full spectrum by one mean frequency is a practical choice. That choice is justified by a small-angle approximation when its hypotheses hold, but it is not an upper bound for every target. The proof therefore states both the convenient predictor and the stronger full-spectrum certificate.

\paragraph{Source and overlap.}
This note preserves the full proof in the supplied \emph{Predicting high-precision bandwidths in tanh MLPs} source, with a reading guide and self-contained source information. It overlaps that supplied note intentionally; it is not an additional independent theorem. It is the bandwidth document to retain in the appendix collection rather than duplicating the older tanh derivations.
...
\paragraph{Provenance of the scalar checks.}
\begingroup\raggedright
The tables and transform checks in this section are transcribed from the supplied bandwidth note. Its text refers to a companion \texttt{check\_bandwidth\_predictions.py}; that script was not included with the supplied source, so this packet does not claim to reproduce its checks. The displayed equations specify the calculations, and the proofs do not depend on the script. The preserved original is \path{predicting_high_precision_bandwidths.tex} in the source packet's \path{supplied/} directory. No new network fit or experimental sweep was performed for this edition.
\par\endgroup
\end{document}
