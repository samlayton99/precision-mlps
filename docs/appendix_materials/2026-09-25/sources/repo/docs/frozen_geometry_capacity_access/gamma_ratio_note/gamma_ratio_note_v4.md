...
Column zero is the bias feature. With \(y_a=f(x_a)/\sqrt m\), the readout coefficients \(v\) minimize \(L(v)=\tfrac12\|B_\gamma v-y\|^2\), half the mean squared training error. Geometry is fixed throughout training. Writing \(r_n=B_\gamma v_n-y\), an ordinary GD update gives
...
## 2. A center integral separates the explicit matrix from the finite corrections
...
![Figure 1. Actual finite kernel matrices.](../../../results/checkpoint_D_optimizers/expD37_capacity_access_figures/gamma_solve/spectrum_mechanism/note_figures/finite_kernels.png)

*Figure 1. Actual matrices \(K_\gamma=B_\gamma B_\gamma^\top\) at \(\gamma=1,4,8,16\), using identical samples and centers and a common color scale. The separation-dependent term produces the diagonal bands. These are full finite matrices, including the bias, grid, and halo contributions.*

## 3. The Fourier integral gives a two-sided finite-network ratio bound
...
These allowances are derived analytically from the Fourier and tanh tails; they are not measured differences from the actual kernel. Appendix C specifies the corrections and the bounds. Write \(\beta_1(\gamma)\ge\cdots\ge\beta_{m-1}(\gamma)\) for the eigenvalues of \(S_\gamma\). Computing this spectrum is part of the method; (7) is not a closed scalar formula for eigenvalue rank versus gamma.

**Theorem 1 (finite-network eigenvalue and ratio interval).** Suppose the centers lie on a lattice of spacing \(h\), the common slope is positive, and \(S_\gamma,\delta_\gamma,\tau_\gamma\) satisfy (9). Let \(0<\ell_\gamma\le\lambda_1(K_\gamma)\le L_\gamma\). For \(2\le i\le m-1\), define
...
The intervals at gamma 4 and 8 are disjoint by more than four orders of magnitude. The Fourier integral supplies an increasing contribution, and the finite corrections and normalization retain this large improvement. This is a checked comparison for the stated geometry. Positivity of (7) alone would not prove that every finite ratio increases: the corrections and the largest eigenvalue also depend on gamma. Appendix E gives an explicit counterexample to that stronger claim.

## 4. The ratio interval bounds both the error and the required number of steps
...
The two eigenvalue bounds have different jobs: upper ratios give a necessary time, while lower ratios give a sufficient time. If a lower ratio endpoint is zero, its entire target weight remains in the upper error curve. An infinite sufficient-time bound means that this bound cannot certify convergence; it does not by itself prove that GD never converges.
...
![Figure 3. Error and step-count intervals from the ratio theorem.](../../../results/checkpoint_D_optimizers/expD37_capacity_access_figures/gamma_solve/spectrum_mechanism/note_interval_figures/full_residual_and_steps.png)

*Figure 3. The finite-kernel spectral error, lower error curve using upper ratio endpoints, and upper error curve using lower endpoints. The 1% threshold determines the step-count interval. Every curve is calculated by eigendecomposition and scalar powers; no long GD run was executed. The bound curves use the actual target projections and the interval theorem, rather than a separate integral-kernel forecast. Appendix F documents numerical allowances and unresolved target energy.*
...
The gamma-4 bound endpoints are rounded outward. These are checked floating-point evaluations of (11)--(12) with the numerical allowances in Appendix F, not certified bounds for the exact tanh kernel or guarantees for a floating-point GD implementation. The necessary bound at gamma 4 is about 91% of the finite-kernel spectral count; the sufficient bound is about 3.3 times that count.

At gamma 4, a separately computed readout achieves relative error \(5.85\times10^{-4}\), below the requested 1%. Thus the long delay is not caused by that tolerance being unrepresentable. The high accuracy exists at the same geometry; ordinary readout GD needs an impractical number of steps to obtain even the much weaker 1% fit. At the larger tested gammas the relevant ratio intervals increase and the necessary and sufficient step counts decrease substantially. Appendix E extends the necessary-delay argument to stable scalar GD schedules.

## Appendix A. The center integral and its restriction
...
## Appendix B. Fourier identity and gamma dependence
...
## Appendix C. Analytic corrections for the lattice and the finite halo
...
### C.2. Bound on unretained lattice terms
...
### C.3. Exterior centers and the matrix enclosure
...
## Appendix D. Eigenvalue endpoints and normalization
...
## Appendix E. What the training bounds imply, and what they do not

### E.1. Error bounds and threshold crossings
...
### E.2. Scalar learning-rate schedules
...
These results concern frozen geometry and Euclidean readout GD, not Adam, preconditioned readout coordinates, or the joint dynamics of gamma.

### E.3. Why the theorem does not claim universal ratio monotonicity
...
## Appendix F. Numerical calculations and reproducibility

### F.1. Geometry, target, and scope
...
### F.2. Computation and numerical allowances
...
### F.3. Proven statements and numerical evidence

| Statement | Status |
|---|---|
| GD recurrence and target-weighted error formula | Exact for the specified frozen-feature updates and zero initial readout. |
