...
- **The solve itself is not new** (it is ELM / random-feature collocation). What this repo adds is the principled geometry and the error theory those methods lack.
...
2. **Append the kernel of $L$ as columns.** Whatever $L$ annihilates (constants and coordinate functions for $\partial^2$; $1,x,y$ are harmonic in 2D) is invisible to the PDE rows and pinned only by BC/IC rows, so give it explicit columns rather than forcing two boundary rows to synthesize a constant from hundreds of tanh columns. The kernel columns are ordinary columns of the dictionary: for Poisson their PDE-block entries are $0$; for Helmholtz $u''+k^2u$ they become $k^2\cdot\mathbf 1$ and $k^2 x$ (nonzero, still known numbers).
...
- **A priori width selection.** $N$ must resolve the unknown solution's scales. The guardrail is a posteriori and cheap: evaluate the residual on fresh points; it certifies the solve without knowing $u^*$.

## Relation to existing work

The solve itself is physics-informed extreme learning machines / random-feature collocation (PIELM; the random feature method of Chen & E; Bacho et al. 2025 "Operator learning at machine precision" is the operator-side cousin, already cited in the workshop paper). Those methods also report near-machine precision on smooth problems. What they take on faith and this repo supplies: *why* a frozen feature geometry works, which geometries do (uniform centers + shared $\lambda^*$ + halo, and their failure modes from Checkpoint C), and the exponential-in-width guarantee (Corollary 1). A paper claim should be positioned as the QI geometry theory extending to PDE collocation, with the diagnosis of why trained PINNs fail as the contrast, not as "lstsq solves Poisson".
...
`uv run --extra dev python docs/pinn_poc.py` (single file, numpy only, ~1 min). The core is ~15 lines: build centers + $\gamma$, form the $L\Phi$ rows and two BC rows, one `lstsq` call.
