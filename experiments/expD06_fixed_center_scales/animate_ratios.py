...
from .ratio_analysis import LABELS
...
def main():
    ...
    for label in args.labels:
        ...
        histories, dense, sources, hashes = [], [], [], {}
        for record in pair:
            ...
            for name in ("history.npz", "dense_parameters.npz"):
        ...
        display = LABELS.get(label, label.replace("_", " "))
        provenance = {"sources": sources, "input_sha256": hashes,
                      "row_order": ["physical w", "physical gamma"], "frames": {}}
        for seed in (0, 1):
            players = []
            for mode, data in [("overview", histories), ("late", dense)]:
                player, steps = render(centers, data, seed, mode, output,
                                       title=f"{args.target}, N={args.n}: {display}")
                players.append(player)
                provenance["frames"][f"s{seed}_{mode}"] = steps
            html = '<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
            html += f'<title>{display}: seed {seed}</title><style>body{{font:16px system-ui;max-width:1200px;margin:auto}} img{{max-width:100%}} button{{padding:5px}}</style><body>'
            html += f'<h1>{display}, seed {seed}: physical readouts and slopes</h1>'
