...
def initial(g, seed, coordinates):
    c, gamma = core.initial_physical(g, seed, "xavier_a_reference")
    return {"z": jnp.asarray(encode(c, g, coordinates)), "lam": jnp.asarray(g.h * gamma),
...
def chunk(n, coordinates, length, capture=False, samples_per_cell=16, batched=True):
    g = core.geometry(n)
    ...
    def advance(state, eta, start):
        def step(current, offset):
            ...
            z1, l1 = current["z"] - eta * gz, current["lam"] - eta * gl
...
@lru_cache(maxsize=8)
def evaluator(n, coordinates, samples_per_cell=16):
    g = core.geometry(n)
    x = jnp.linspace(-1, 1, samples_per_cell*n+1)
...
def case_key(case):
    return f"N{case['n']}_s{case['seed']}_{case['coordinates']}_eta{case['eta']:.8g}"
...
def advance_group(output, cases, frontier, deadline=float("inf"), samples_per_cell=16):
    ...
    for case, path in zip(cases, paths):
        ...
        metadata = dict(case, optimizer="gd", target="sine", initialization="xavier_a_reference",
    ...
    def save():
        ...
        arrays = jax.device_get(evaluator(n, coord, samples_per_cell)(state))
    ...
    while at < frontier and time.monotonic() < deadline:
        ...
        state, result = chunk(n, coord, length, capture, samples_per_cell)(state, eta, at)
