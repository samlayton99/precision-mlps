"""Experiment 0D: optimal lambda vs target frequency, QI vs lstsq.
...
Usage:
    python3 experiments/exp0D_lambda_frequency/run.py            # collect + plot
    python3 experiments/exp0D_lambda_frequency/run.py --plot      # plot only
"""
...
RESULTS_DIR = REPO_ROOT / "results" / "exp0D_lambda_frequency"
...
def _plot_optimum(rows):
    ...
    ax.set_title("exp0D: optimal bandwidth vs frequency (fp64)")
...
def _plot_curves(rows):
    ...
    fig.suptitle("exp0D: error vs lambda (solid = lstsq, dashed = QI)", fontsize=13, y=0.99)
    caption = (
