...
the QI-dependent metrics of exp0B (ratio_full/ratio_row/qi_fit) do not apply. We
...
Usage:
    python3 experiments/exp0C_gelu_conditioning/run.py            # collect + plot
    python3 experiments/exp0C_gelu_conditioning/run.py --plot      # plot only
"""
...
RESULTS_DIR = REPO_ROOT / "results" / "exp0C_gelu_conditioning"
...
def _plot_step1(data):
    ...
    ax.set_title(rf"exp0C step 1: lstsq error vs $\lambda$ on $\sin(\pi x)$, N={data['N_step1']}")
    ...
    caption = (
        ...
        "cancellation wall (see exp0D/exp01). The dotted line marks the GELU "
...
def _plot_step2(data):
    ...
    fig.suptitle(rf"exp0C step 2: GELU vs tanh conditioning at $\lambda$={best_lambda:.3f} (fp64)",
                 fontsize=14, y=0.99)
    caption = (
