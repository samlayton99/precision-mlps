"""Measure the real-analytic smooth absolute value for the combined figure."""
import hashlib
import json
...
OUT = ROOT / "results/checkpoint_C_geometry/expC09_bandwidth_figures/smooth_absolute"
LABEL = "Smooth absolute value"
FORMULA = "log(cosh(10*x)) / 10"


def target(x):
    return np.log(np.cosh(10*x))/10
...
def measure(width, config):
    ...
    row = {"target": "smooth_absolute", "width": width, "N": n,
...
def main():
    ...
    manifest = {"formula": FORMULA, "config": dict(config, targets=["smooth_absolute"]),
