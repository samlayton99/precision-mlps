"""Experiment 0B: How close are the QI and least-squares readouts?
...
Usage:
    python3 experiments/exp0B_coeff_diff/run.py            # collect + plot
    python3 experiments/exp0B_coeff_diff/run.py --plot      # plot only
"""
...
RESULTS_DIR = REPO_ROOT / "results" / "exp0B_coeff_diff"
...
def _ratio_figure(rows, out_path):
    ...
    ax.set_title("exp0B: QI vs lstsq coefficient deviation -- full space vs row space")
    ax.grid(True, which="both", alpha=0.3)
...
def plot_results(data_path=None):
    ...
    _two_panel(
        ...
        title=fr"exp0B: QI vs lstsq coefficient closeness vs width ($\lambda$={lam}, fp64)",
        caption=(
    ...
    _two_panel(
        ...
        title=fr"exp0B: closeness vs lstsq sample count ($\lambda$={lam}, fp64, target={NSAMPLES_TARGET})",
        caption=(
